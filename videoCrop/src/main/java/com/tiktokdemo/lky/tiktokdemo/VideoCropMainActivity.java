...
public class VideoCropMainActivity extends AppCompatActivity implements View.OnClickListener {

    private final String LOCAL_MUSIC_NAME = "RISE.mp3";
    private final String LOCAL_VIDEO_NAME = "RBB.mp4";
    private String mLocalMusicPath = VideoCropConstant.PIC_FILE + File.separator + LOCAL_MUSIC_NAME;
//    private String mLocalVideoPath = VideoCropConstant.PIC_FILE + File.separator + LOCAL_VIDEO_NAME;
    private String mLocalVideoPath = Environment.getExternalStorageDirectory()  + File.separator + "/新文件夹/09地狱之门.mp4";
    ...
    @Override public void onClick(View v) {
        int i = v.getId();
        if (i == R.id.main_record_btn) {
            if (!FileUtils.checkFileExits(mLocalMusicPath)) {
                FileUtils.copyFileFromAssets(this, LOCAL_MUSIC_NAME, VideoCropConstant.PIC_FILE);
            }
        ...
        } else if (i == R.id.main_crop_btn) {
            if (!FileUtils.checkFileExits(mLocalVideoPath)) {
                FileUtils.copyFileFromAssets(this, LOCAL_VIDEO_NAME, VideoCropConstant.PIC_FILE);
            }
