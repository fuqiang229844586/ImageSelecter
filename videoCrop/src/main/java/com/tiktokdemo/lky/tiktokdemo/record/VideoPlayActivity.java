...
import com.tiktokdemo.lky.tiktokdemo.VideoCropMainActivity;
...
public class VideoPlayActivity extends Activity implements View.OnClickListener, VideoPlayContract.View {
    ...
    public void clearCacheDataAndStartMainActivity(boolean isJumpHome){
        ...
        Intent intent = new Intent(this, VideoCropMainActivity.class);
    ...
    public void onClick(View v) {
        int i = v.getId();
        if (i == R.id.tidal_pat_record_voice_complete_btn) {
        ...
        } else if (i == R.id.tidal_pat_record_video_back_img) {
            onBackPressed();

        } else if (i == R.id.tidal_pat_record_video_cut_music_img) {
            if (mTidalPatRecordDraftBean.getRecordTimeType() != RecordTimeType.RECORD_TIME_15) {
                return;
            }
        ...
        } else if (i == R.id.tidal_pat_record_cut_audio_confirm_img) {
            if (mPresenter.isAudioCuting()) {
                return;
            }
            hideCutAudioLayout();

        } else if (i == R.id.tidal_pat_upload_video_volume_img) {
            if (mTidalPatRecordDraftBean.getRecordTimeType() != RecordTimeType.RECORD_TIME_15) {
                return;
            }
            showVolumeLayout();

        } else if (i == R.id.tidal_pat_record_volume_layout) {
            hideVolumeLayout();

        } else if (i == R.id.tidal_pat_upload_video_special_effects_img) {
            if (mTidalPatRecordDraftBean.getRecordTimeType() != RecordTimeType.RECORD_TIME_15) {
                return;
            }
            inSpecialEffectsMode();

        } else if (i == R.id.personal_show_record_video_loading_layout) {
        }
    }
    ...
    public void saveVideo(final TidalPatRecordDraftBean tidalPatRecordDraftBean) {
        if(TextUtils.isEmpty(tidalPatRecordDraftBean.getVideoName())){
            FileUtils.copyFile(mTidalPatRecordDraftBean.getVideoLocalUrl(), VideoCropConstant.RECORD_VIDEO_PATH,System.currentTimeMillis() + ".mp4");
            ToastTool.showShort(this,"视频已保存至：" + VideoCropConstant.RECORD_VIDEO_PATH + File.separator + System.currentTimeMillis() + ".mp4");
    ...
    class SpecialEffectsViewClickListener implements View.OnClickListener{
        ...
        public void onClick(View v) {
            ...
            int i = v.getId();
            if (i == R.id.tidal_pat_upload_se_bottom_filter_txt) {
            ...
            } else if (i == R.id.tidal_pat_upload_se_bottom_time_txt) {
            ...
            } else if (i == R.id.tidal_pat_upload_se_save_txt) {
                outSpecialEffectsMode(true);

            } else if (i == R.id.tidal_pat_upload_se_cancel_txt) {
                outSpecialEffectsMode(false);

            } else if (i == R.id.tidal_pat_record_video_upload_pv) {
                if (isSpecialEffectsEditMode) {
                    mVideoPlayView.changeState();
                }

            } else if (i == R.id.tidal_pat_upload_se_remove_txt) {
