...
import com.cameralibrary.util.AngleUtil;
import com.cameralibrary.util.CameraParamUtil;
import com.cameralibrary.util.CheckPermission;
import com.cameralibrary.util.DeviceUtil;
import com.cameralibrary.util.FileUtil;
import com.cameralibrary.util.LogUtil;
import com.cameralibrary.util.ScreenUtils;
...
public class CameraInterface implements Camera.PreviewCallback {
    ...
    public void setZoom(float zoom, int type) {
        ...
        try {
            ...
            LogUtil.i("setZoom => zoom " + zoom + " nowScaleRate => " + nowScaleRate);
        } catch (Exception e) {
            e.printStackTrace();
...
//        mCamera = Camera.open();
    ...
    public void doStartPreview(SurfaceHolder holder, float screenProp) {
        ...
        if (mCamera != null) {
            try {
                mParams = mCamera.getParameters();
                Camera.Size previewSize = CameraParamUtil.getInstance().getPreviewSize(mParams.getSupportedPreviewSizes(), 1000, screenProp);
                ...
                isPreviewing = true;
                previewProp = 1f * previewSize.width / previewSize.height;
