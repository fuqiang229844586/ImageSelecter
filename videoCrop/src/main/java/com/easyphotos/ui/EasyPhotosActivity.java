...
import com.easyphotos.ui.adapter.AlbumItemsAdapter;
import com.easyphotos.ui.adapter.PhotosAdapter;
import com.easyphotos.ui.widget.PressedTextView;
import com.easyphotos.utils.color.ColorUtils;
import com.easyphotos.utils.media.DurationUtils;
import com.easyphotos.utils.media.MediaScannerConnectionUtils;
import com.easyphotos.utils.permission.PermissionUtil;
import com.easyphotos.utils.settings.SettingsUtils;
...
//    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
            ...
            case RESULT_OK:
                ...
                if (Code.REQUEST_PREVIEW_ACTIVITY == requestCode) {
                    ...
                    if (data.getBooleanExtra(Key.PREVIEW_CLICK_DONE, false)) {
                        String localVideoPath = data.getStringExtra(Key.EXTRA_RESULT_CAPTURE_VIDEO_PATH);
                        ArrayList<Photo> resultPhotos = new ArrayList<>();
                        resultPhotos.add(new Photo("", localVideoPath, 0, 0, 0, 0, 0, ""));
                        resultList.addAll(resultPhotos);
                        done(false);
                    }
                    return;
                }
        ...
        if (requestCode == VideoCropConstant.HOLDER_CROP_REQUEST_CODE) {//剪裁视频
            ...
            resultPhotos.add(new Photo("", localVideoPath, 0, 0, 0, 0, 0, "video"));
            resultPaths.add(localVideoPath);
            Intent intent = new Intent();
            intent.putParcelableArrayListExtra(EasyPhotos.RESULT_PHOTOS, resultPhotos);
            intent.putStringArrayListExtra(EasyPhotos.RESULT_PATHS, resultPaths);
            intent.putExtra(EasyPhotos.RESULT_SELECTED_ORIGINAL, Setting.selectedOriginal);
            setResult(RESULT_OK, intent);
            finish();
...
//            }
    ...
    private void done(boolean isClear) {
        Intent intent = new Intent();
        ...
        if(resultList.size() > 1){
            ...
            resultPaths.add(resultList.get(0).path);
        }
        intent.putStringArrayListExtra(EasyPhotos.RESULT_PATHS, resultPaths);
        intent.putExtra(EasyPhotos.RESULT_SELECTED_ORIGINAL, Setting.selectedOriginal);
        if (Setting.isCrop) {
            ...
            if (null != resultList.get(0)) {
                ...
                if (photo.type.endsWith("mp4")) {
                    ...
                    if ((photo.duration / 1000) > VideoCropConstant.VIDEO_LENGTH) {//视频时间超过三分钟去剪裁
                        AlertDialog.Builder builder = new AlertDialog.Builder(EasyPhotosActivity.this);
                        ...
                        builder.setPositiveButton(getString(R.string.video_crop_confirm), new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                if (!FileUtils.checkFileExits(localVideoPath)) {
                                    FileUtils.copyFileFromAssets(EasyPhotosActivity.this, localVideoPath, VideoCropConstant.PIC_FILE);
                                }
                                startActivityForResult(new Intent(EasyPhotosActivity.this, VideoCropActivity.class)
